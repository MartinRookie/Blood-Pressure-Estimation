function [qrs_amp_raw,qrs_i_raw,delay,ecg_h]=pan_tompkin(ecg,fs,gr,window_size)
%% Some Parameters needs to be changed:
   % round(window_size*fs), the minimum distance to find a peak
   % (1/fs)*(window_size*fs), the meaning of windows size of the minimum
   % time difference for finding a peak
...
% 200 msec distance
[pks,locs] = findpeaks(ecg_m,'MinPeakHeight', mean(ecg_m),'MINPEAKDISTANCE',round(window_size*fs));
